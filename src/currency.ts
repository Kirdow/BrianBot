...
async function renewCache(): Promise<Record<string, number>> {
    const data: Record<string, number> = (await getCurrencyJson())?.eur
    if (!data) return {}
...
async function getCache(): Promise<Record<string, number>> {
...
export async function getCurrency(name: string): Promise<number> {
